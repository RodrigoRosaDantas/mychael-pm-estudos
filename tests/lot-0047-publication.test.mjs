...
test('LOT-0047 monta catálogo v46 sem reduzir validações', () => {
  assert.deepEqual(validateCatalog(catalog), []);
  assert.equal(lot.lotId, 'LOT-0047');
  assert.equal(lot.contentVersion, 46);
  assert.equal(catalog.contentVersion, 46);
  assert.equal(catalog.publication.lotId, 'LOT-0047');
  assert.equal(catalog.units.length, 46);
  assert.equal(catalog.materials.length, 46);
  assert.equal(catalog.questions.length, 315);
  assert.equal(catalog.questionSets.length, 46);
});
...
test('U046 preserva fonte normativa vigente e separação teoria/questões', () => {
  ...
  ids.forEach((id, index) => {
    ...
    assert.ok(q, id); assert.equal(q.unitId, 'U046', id); assert.equal(q.answer, answers[index], id); assert.equal(q.options.length, 5, id); assert.ok(q.options.some((o) => o.id === answers[index]), id); assert.deepEqual(q.sourceIds, ['FNT-0027'], id); assert.ok(q.commentary.length > 20, id); assert.ok(q.foundation.length > 20, id); assert.equal(q.annulled, false, id); assert.equal(q.duplicateOf, null, id); assert.equal(q.imageRequired, false, id); assert.equal(q.valid, true, id);
  });
  const set = find(catalog.questionSets, 'QS046');
  assert.ok(set); assert.equal(set.unitId, 'U046'); assert.deepEqual(set.questionIds, ids); assert.equal(set.correctionMode, 'Ao finalizar'); assert.equal('questions' in set, false); assert.equal('answers' in set, false);
});

test('U046 tem aplicabilidade explícita PMDF + PMGO sem ativar PMMG', () => {
  const rule = applicability.unitApplicability.find((item) => item.unitId === 'U046');
  assert.ok(rule); assert.equal(rule.classification, 'shared-2'); assert.deepEqual(rule.competitions, ['PMDF','PMGO']); assert.equal(rule.status, 'verified-not-pmmg'); assert.deepEqual(rule.evidenceIds, ['FNT-0015','FNT-0014','FNT-0027']);

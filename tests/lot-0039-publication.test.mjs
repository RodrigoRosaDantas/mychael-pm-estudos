...
test('LOT-0039 permanece íntegro após a evolução do catálogo para v38', () => {
  assert.deepEqual(validateCatalog(catalog), []);
  assert.equal(lot0039.contentVersion, 36);
  assert.equal(lot0039.lotId, 'LOT-0039');
  assert.equal(catalog.contentVersion, 38);
  assert.equal(catalog.publication.lotId, 'LOT-0042');
  assert.equal(catalog.units.length, 38);
  assert.equal(catalog.materials.length, 38);
  assert.equal(catalog.questions.length, 259);
  assert.equal(catalog.questionSets.length, 38);
});

test('U039 preserva LINDB específica PMMG e rotação definitiva desativada', () => {
  const subject = find(catalog.subjects, 'MAT-NDIR');
  const parent = find(catalog.topics, 'ASS-NDIR-001');
  const child = find(catalog.topics, 'ASS-NDIR-001-01');
  const source = find(catalog.sources, 'FNT-0036');
  assert.ok(subject); assert.deepEqual(subject.coverage, ['PMMG']);

...
test('LOT-0009 amplia o catálogo cumulativo sem relações quebradas', () => {
  ...
  assert.deepEqual(catalog.units.map(({ id }) => id), ['U001','U002','U003','U004','U005','U006','U007','U008','U009']);
});
...
test('fonte FNT-0013 e manifesto público correspondem ao catálogo exportado', () => {
  ...
  assert.equal(manifest.schemaVersion, 1); assert.equal(manifest.contentVersion, 9);

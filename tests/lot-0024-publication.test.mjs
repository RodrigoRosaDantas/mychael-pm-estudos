...
test('LOT-0024 permanece íntegro no catálogo cumulativo posterior', () => {
  assert.deepEqual(validateCatalog(catalog), []);
  assert.ok(catalog.contentVersion >= 20);
  assert.ok(find(catalog.units,'U024'));
  assert.ok(find(catalog.materials,'M024'));
  assert.ok(find(catalog.questionSets,'QS024'));
});

...
test('LOT-0018 permanece íntegro no catálogo cumulativo posterior', () => {
  assert.deepEqual(validateCatalog(catalog), []);
  assert.ok(catalog.contentVersion >= 18);
  assert.ok(findById(catalog.units, 'U018'));
  assert.ok(findById(catalog.materials, 'M018'));
  assert.ok(findById(catalog.questionSets, 'QS018'));
});

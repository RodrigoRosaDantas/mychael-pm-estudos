...
test('U003 referencia taxonomia, fontes e componentes corretos', () => {
  const unit = findById(catalog.units, 'U003');
  assert.ok(unit);
...
test('U004 referencia taxonomia, fontes e componentes corretos', () => {
  const unit = findById(catalog.units, 'U004');
  assert.ok(unit);
...
test('M003 e M004 mantêm teoria separada das questões e orientação de revisão', () => {
  for (const id of ['M003', 'M004']) {
    const material = findById(catalog.materials, id);
    assert.ok(material, id);
    assert.equal(material.required, true, id);
    assert.equal('questionIds' in material, false, id);
    assert.equal('answer' in material, false, id);
    assert.equal('options' in material, false, id);
    const headings = material.blocks.filter(({ type }) => type === 'heading').map(({ text }) => text);
    assert.ok(headings.some((heading) => heading.includes('Erros comuns')), id);
    assert.ok(headings.some((heading) => heading.includes('Resumo da aula')), id);
    assert.ok(headings.some((heading) => heading.includes('Orientação de revisão')), id);
    assert.ok(headings.some((heading) => heading.includes('Fontes editoriais')), id);
  }
});
...
test('as doze novas questões possuem gabaritos e integridade editorial auditados', () => {
  ...
  for (const [id, answer] of Object.entries(expected)) {
    ...
    assert.deepEqual(question.sourceIds, ['FNT-0016', 'FNT-0017'], id);
...
test('QS003 e QS004 apenas referenciam o Banco Mestre', () => {
  const expected = {
    QS003: ['Q000012', 'Q000013', 'Q000014', 'Q000015', 'Q000016', 'Q000017'],
    QS004: ['Q000018', 'Q000019', 'Q000020', 'Q000021', 'Q000022', 'Q000023']
  };
  for (const [id, questionIds] of Object.entries(expected)) {
    const set = findById(catalog.questionSets, id);
    assert.ok(set, id);
    assert.deepEqual(set.questionIds, questionIds, id);
    assert.equal(set.correctionMode, 'Por questão', id);
    assert.equal('questions' in set, false, id);
    assert.equal('answers' in set, false, id);
  }
});

test('FNT-0016 e FNT-0017 são referências institucionais históricas verificadas', () => {
  for (const id of ['FNT-0016', 'FNT-0017']) {
    const source = findById(catalog.sources, id);
    assert.ok(source, id);
    assert.equal(source.official, true, id);
    assert.equal(source.temporalStatus, 'Histórica', id);
    assert.equal(source.lastVerifiedAt, '2026-08-04', id);
    assert.match(source.organization, /CECIERJ/, id);
  }
});

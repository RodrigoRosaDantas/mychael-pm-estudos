...
test('LOT-0007 permanece íntegro no catálogo cumulativo v11', () => {
  ...
  assert.equal(catalog.units.length, 11); assert.equal(catalog.materials.length, 11); assert.equal(catalog.questions.length, 70); assert.equal(catalog.questionSets.length, 11); assert.ok(findById(catalog.subjects, 'MAT-DCON'));
});

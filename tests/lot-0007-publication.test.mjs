...
test('LOT-0007 integra Direito Constitucional ao catálogo cumulativo válido', () => {
  ...
  assert.ok(findById(catalog.subjects, 'MAT-DCON'));
});
test('U007 referencia taxonomia, fontes e componentes auditados', () => {
  const unit = findById(catalog.units, 'U007');
  assert.ok(unit);
  assert.equal(unit.subjectId, 'MAT-DCON');
  assert.deepEqual(unit.topicIds, ['ASS-DCON-001', 'ASS-DCON-001-01']);
  assert.deepEqual(unit.sourceIds, ['FNT-0001', 'FNT-0023']);
  assert.deepEqual(unit.materialIds, ['M007']);
  assert.deepEqual(unit.questionIds, ['Q000037', 'Q000038', 'Q000039', 'Q000040', 'Q000041', 'Q000042']);
  assert.deepEqual(unit.questionSetIds, ['QS007']);
});
test('M007 mantém teoria separada das questões e orientação de revisão', () => {
  const material = findById(catalog.materials, 'M007');
  assert.ok(material);
  assert.equal(material.unitId, 'U007');
...
test('seis questões da U007 mantêm gabaritos e integridade editorial', () => {
  ...
  for (const [id, answer] of Object.entries(expected)) {
    ...
    assert.equal(question.unitId, 'U007', id);
    assert.equal(question.subjectId, 'MAT-DCON', id);
    assert.deepEqual(question.sourceIds, ['FNT-0001', 'FNT-0023'], id);
...
test('QS007 apenas referencia questões do Banco Mestre na ordem auditada', () => {
  const set = findById(catalog.questionSets, 'QS007');
  assert.ok(set);
  assert.equal(set.unitId, 'U007');
  assert.equal(set.correctionMode, 'Por questão');
  assert.deepEqual(set.questionIds, ['Q000037', 'Q000038', 'Q000039', 'Q000040', 'Q000041', 'Q000042']);
  assert.equal('questions' in set, false);
  assert.equal('answers' in set, false);
});
test('fontes constitucionais preservam hierarquia e situação temporal', () => {
  const constitution = findById(catalog.sources, 'FNT-0001');
  const senate = findById(catalog.sources, 'FNT-0023');
  assert.equal(constitution.official, true);
  assert.equal(constitution.priority, 'Primária');
  assert.equal(constitution.temporalStatus, 'Vigente');
  assert.equal(senate.official, true);
  assert.equal(senate.priority, 'Secundária');
  assert.equal(senate.temporalStatus, 'Histórica');
});

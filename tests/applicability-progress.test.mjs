...
test('camada de aplicabilidade preserva guardrails e não infere PMMG', () => {
  ...
  assert.deepEqual(
    ...
    ['U001','U002','U003','U004','U005','U006','U008','U009','U013','U020','U021','U022','U023','U024','U026','U027','U028','U029','U030']
  );
  ...
  for (const id of ['U006','U013','U026','U027','U028','U029','U030']) {
    const rule = applicability.unitApplicability.find((item) => item.unitId === id);
    assert.equal(rule.classification, 'common-3');
    assert.equal(rule.status, 'verified');
    assert.equal(rule.competitions.includes('PMMG'), true);
    assert.ok(rule.evidenceIds.length > 0, `${id} precisa preservar evidência temática explícita`);
  }
});
...
test('erro aberto impede que unidade marcada completed conte como concluída', () => {
  const catalog = { units: [{ id: 'U001', order: 10, coverage: ['PMGO', 'PMDF'] }], questions: [{ id: 'Q1', unitId: 'U001' }] };
  const model = computeCompetitionProgress({ catalog, applicability, studyUnits: [{ unit_id: 'U001', status: 'completed', mastery_percent: 100 }], attempts: [{ question_id: 'Q1', is_correct: false, answered_at: '2026-08-07T11:00:00Z' }], openErrorQuestionIds: ['Q1'] });
...
test('próximo passo prioriza correção antes de qualquer conteúdo novo', () => {
  const catalog = { units: [{ id:'U001',order:10,title:'Primeira comum'},{id:'U002',order:20,title:'Segunda comum'},{id:'U015',order:30,title:'Penal compartilhado'}], questions:[{id:'Q1',unitId:'U001'},{id:'Q2',unitId:'U001'},{id:'Q3',unitId:'U002'}] };
  ...
  assert.equal(correction.phase,'correction'); assert.equal(correction.phaseLabel,'Corrigir antes de avançar'); assert.equal(correction.unit.id,'U001'); assert.equal(correction.pendingErrors,1);
  const afterCorrection = nextGuidedStep({ catalog, applicability, studyUnits, openErrorQuestionIds:[] });
  assert.equal(afterCorrection.phase,'common-3'); assert.equal(afterCorrection.unit.id,'U002');
});

test('entre vários erros o ciclo escolhe primeiro a unidade pedagogicamente mais antiga', () => {
  const catalog = { units:[{id:'U001',order:10,title:'Primeira comum'},{id:'U002',order:20,title:'Segunda comum'}], questions:[{id:'Q1',unitId:'U001'},{id:'Q2',unitId:'U002'}] };
  const step = nextGuidedStep({ catalog, applicability, studyUnits:[], openErrorQuestionIds:['Q2','Q1','Q999'] });
  assert.equal(step.phase,'correction'); assert.equal(step.unit.id,'U001'); assert.equal(step.pendingErrors,1);
});
...
test('próximo passo respeita núcleo comum antes de convergência por dois quando não há erro', () => {
  ...
  const first=nextGuidedStep({catalog,applicability,studyUnits:[]}); assert.equal(first.phase,'common-3'); assert.equal(first.unit.id,'U001');
  const afterFirst=nextGuidedStep({catalog,applicability,studyUnits:[{unit_id:'U001',status:'completed'}]}); assert.equal(afterFirst.phase,'common-3'); assert.equal(afterFirst.unit.id,'U002');
  const afterCommon=nextGuidedStep({catalog,applicability,studyUnits:[{unit_id:'U001',status:'completed'},{unit_id:'U002',status:'completed'}]}); assert.equal(afterCommon.phase,'shared-2'); assert.equal(afterCommon.unit.id,'U015');
});

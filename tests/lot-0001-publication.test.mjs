...
function collectKeys(value, keys = []) {
  ...
  else if (value && typeof value === 'object') for (const [key, child] of Object.entries(value)) { keys.push(key); collectKeys(child, keys); }
  return keys;
}

test('catálogo cumulativo autorizado permanece válido até o LOT-0023', () => {
...
test('LOT-0001 permanece íntegro após a inclusão das unidades posteriores', () => {
  ...
  const answers = Object.fromEntries(catalog.questions.filter(({ id }) => /^Q00000[1-5]$/.test(id)).map(({ id, answer }) => [id, answer]));
  assert.deepEqual(answers, { Q000001: 'C', Q000002: 'A', Q000003: 'D', Q000004: 'B', Q000005: 'E' });
});

test('material M001 preserva a sequência pedagógica completa', () => {
  const headings = findById(catalog.materials, 'M001').blocks.filter(({ type }) => type === 'heading').map(({ text }) => text);
  assert.deepEqual(headings, ['1. Comece por uma ideia simples','2. Informação explícita','3. Informação implícita','4. Diferença prática','5. Método em quatro passos','6. Exemplos comentados','7. Explicação técnica','8. Erros comuns','9. Resumo da aula','10. Orientação de revisão','11. Fontes editoriais']);
});
...
test('catálogo não contém dados pessoais nem segredos', () => {
  ...
  assert.deepEqual(collectKeys(catalog).filter((key) => forbidden.has(key)), []);

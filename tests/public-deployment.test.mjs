...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 50); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 50); assert.equal(result.questions, 343);
  assert.equal(result.coverage.unitCounts.PMDF, 46); assert.equal(result.coverage.unitCounts.PMGO, 37); assert.equal(result.coverage.unitCounts.PMMG, 31); assert.equal(result.coverage.questionCounts.PMDF, 315); assert.equal(result.coverage.questionCounts.PMGO, 252); assert.equal(result.coverage.questionCounts.PMMG, 211); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.specificRotationEnabled, false); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,50); assert.equal(result.questions,343); assert.equal(result.coverage.questionCounts.PMDF,315); assert.equal(result.coverage.questionCounts.PMGO,252); assert.equal(result.coverage.questionCounts.PMMG,211); assert.equal(result.coverage.specificRotationEnabled,false);
});

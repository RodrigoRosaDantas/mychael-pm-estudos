...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 41); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 41); assert.equal(result.questions, 280);
  assert.equal(result.coverage.unitCounts.PMDF, 37); assert.equal(result.coverage.unitCounts.PMGO, 33); assert.equal(result.coverage.unitCounts.PMMG, 26); assert.equal(result.coverage.questionCounts.PMDF, 252); assert.equal(result.coverage.questionCounts.PMGO, 224); assert.equal(result.coverage.questionCounts.PMMG, 176); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.specificRotationEnabled, false); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,41); assert.equal(result.questions,280); assert.equal(result.coverage.questionCounts.PMDF,252); assert.equal(result.coverage.questionCounts.PMGO,224); assert.equal(result.coverage.questionCounts.PMMG,176); assert.equal(result.coverage.specificRotationEnabled,false);
});

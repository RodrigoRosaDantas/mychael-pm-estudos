...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 38); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 38); assert.equal(result.questions, 259);
  assert.equal(result.coverage.unitCounts.PMDF, 34); assert.equal(result.coverage.unitCounts.PMGO, 30); assert.equal(result.coverage.unitCounts.PMMG, 26); assert.equal(result.coverage.questionCounts.PMDF, 231); assert.equal(result.coverage.questionCounts.PMGO, 203); assert.equal(result.coverage.questionCounts.PMMG, 176); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.specificRotationEnabled, false); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,38); assert.equal(result.questions,259); assert.equal(result.coverage.questionCounts.PMDF,231); assert.equal(result.coverage.questionCounts.PMGO,203); assert.equal(result.coverage.questionCounts.PMMG,176); assert.equal(result.coverage.specificRotationEnabled,false);
});

...
} from '../scripts/check-public-deployment.mjs';
...
test('detecta arquivo público desatualizado', () => {
  ...
  assert.throws(
    ...
    /Deploy desatualizado ou divergente em content\/catalog\.json/
  );
});
...
test('validação remota compara o deploy com o commit atual', async () => {
  ...
  const result = await checkPublicDeployment('https://example.test/mychael-pm-estudos', {
    expectedFiles,
    fetchImpl,
    attempts: 1,
    delayMs: 0
...
test('validação remota falha quando o site não responde', async () => {
  ...
  await assert.rejects(
    checkPublicDeployment('https://example.test/mychael-pm-estudos', {
      expectedFiles,
      fetchImpl,
      attempts: 1,
      delayMs: 0

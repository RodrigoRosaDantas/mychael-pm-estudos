...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 49); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 49); assert.equal(result.questions, 336);
  assert.equal(result.coverage.unitCounts.PMDF, 45); assert.equal(result.coverage.unitCounts.PMGO, 36); assert.equal(result.coverage.unitCounts.PMMG, 31); assert.equal(result.coverage.questionCounts.PMDF, 308); assert.equal(result.coverage.questionCounts.PMGO, 245); assert.equal(result.coverage.questionCounts.PMMG, 211); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.specificRotationEnabled, false); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,49); assert.equal(result.questions,336); assert.equal(result.coverage.questionCounts.PMDF,308); assert.equal(result.coverage.questionCounts.PMGO,245); assert.equal(result.coverage.questionCounts.PMMG,211); assert.equal(result.coverage.specificRotationEnabled,false);
});

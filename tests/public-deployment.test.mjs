...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 46); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 46); assert.equal(result.questions, 315);
  assert.equal(result.coverage.unitCounts.PMDF, 42); assert.equal(result.coverage.unitCounts.PMGO, 35); assert.equal(result.coverage.unitCounts.PMMG, 29); assert.equal(result.coverage.questionCounts.PMDF, 287); assert.equal(result.coverage.questionCounts.PMGO, 238); assert.equal(result.coverage.questionCounts.PMMG, 197); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.specificRotationEnabled, false); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,46); assert.equal(result.questions,315); assert.equal(result.coverage.questionCounts.PMDF,287); assert.equal(result.coverage.questionCounts.PMGO,238); assert.equal(result.coverage.questionCounts.PMMG,197); assert.equal(result.coverage.specificRotationEnabled,false);
});

...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 22); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 22); assert.equal(result.questions, 147);
  assert.equal(result.coverage.unitCounts.PMDF, 22); assert.equal(result.coverage.unitCounts.PMGO, 22); assert.equal(result.coverage.unitCounts.PMMG, 16); assert.equal(result.coverage.questionCounts.PMMG, 106); assert.equal(result.coverage.commonUnits, 16); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,22); assert.equal(result.questions,147); assert.equal(result.coverage.questionCounts.PMMG,106);
});
test('validação remota falha quando o site não responde', async () => { const fetchImpl=async()=>new Response('indisponível',{status:503}); await assert.rejects(checkPublicDeployment('https://example.test/mychael-pm-estudos',{expectedFiles,fetchImpl,attempts:1,delayMs:0}),/Falha HTTP 503/); });

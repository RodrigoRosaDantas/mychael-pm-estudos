...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 27); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 27); assert.equal(result.questions, 182);
  assert.equal(result.coverage.unitCounts.PMDF, 27); assert.equal(result.coverage.unitCounts.PMGO, 25); assert.equal(result.coverage.unitCounts.PMMG, 21); assert.equal(result.coverage.questionCounts.PMMG, 141); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,27); assert.equal(result.questions,182); assert.equal(result.coverage.questionCounts.PMMG,141);
});

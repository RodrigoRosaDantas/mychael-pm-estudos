...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 47); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 47); assert.equal(result.questions, 322);
  assert.equal(result.coverage.unitCounts.PMDF, 43); assert.equal(result.coverage.unitCounts.PMGO, 35); assert.equal(result.coverage.unitCounts.PMMG, 30); assert.equal(result.coverage.questionCounts.PMDF, 294); assert.equal(result.coverage.questionCounts.PMGO, 238); assert.equal(result.coverage.questionCounts.PMMG, 204); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.specificRotationEnabled, false); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,47); assert.equal(result.questions,322); assert.equal(result.coverage.questionCounts.PMDF,294); assert.equal(result.coverage.questionCounts.PMGO,238); assert.equal(result.coverage.questionCounts.PMMG,204); assert.equal(result.coverage.specificRotationEnabled,false);
});

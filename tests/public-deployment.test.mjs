...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 34); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 34); assert.equal(result.questions, 231);
  assert.equal(result.coverage.unitCounts.PMDF, 34); assert.equal(result.coverage.unitCounts.PMGO, 29); assert.equal(result.coverage.unitCounts.PMMG, 23); assert.equal(result.coverage.questionCounts.PMDF, 231); assert.equal(result.coverage.questionCounts.PMMG, 155); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.specificRotationEnabled, false); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,34); assert.equal(result.questions,231); assert.equal(result.coverage.questionCounts.PMDF,231); assert.equal(result.coverage.questionCounts.PMMG,155); assert.equal(result.coverage.specificRotationEnabled,false);
});

...
test('arquivos locais formam uma publicação íntegra e multi-concurso', () => {
  ...
  assert.equal(result.contentVersion, 25); assert.equal(result.publicationStatus, 'published'); assert.equal(result.pages, 12); assert.equal(result.units, 25); assert.equal(result.questions, 168);
  assert.equal(result.coverage.unitCounts.PMDF, 25); assert.equal(result.coverage.unitCounts.PMGO, 25); assert.equal(result.coverage.unitCounts.PMMG, 19); assert.equal(result.coverage.questionCounts.PMMG, 127); assert.equal(result.coverage.commonUnits, 19); assert.equal(result.coverage.pmmgTopicReviewPending, 0); assert.deepEqual(result.coverage.unclassifiedUnits, []);
});
...
test('validação remota compara o deploy completo com o commit atual', async () => {
  ...
  assert.equal(result.attempt,1); assert.equal(result.pages,12); assert.equal(result.units,25); assert.equal(result.questions,168); assert.equal(result.coverage.questionCounts.PMMG,127);
});

...
const panels = await readFile(path.join(root, 'assets/progress-panels.js'), 'utf8');
...
const requiredFiles = ['assets/styles.css','assets/app.js','assets/progress-panels.js','assets/progress-panels.css','assets/supabase-config.js','content/catalog.json','.nojekyll'];
...
for (const id of ['publicationBadge','heroDescription','unitWorkspace','questionWorkspace','studyProgress','reviewWorkspace','errorWorkspace','performanceWorkspace']) {
  if (!html.includes(`id="${id}"`)) throw new Error(`Controle funcional ausente: ${id}`);
}
...
if (/signUp\s*\(|Cadastrar|Criar conta/i.test(app + panels + html)) throw new Error('Cadastro público não pode ser oferecido.');
...
for (const table of ['review_items','error_items','question_attempts','study_units']) {
  if (!panels.includes(`from('${table}')`)) throw new Error(`Painel privado sem integração com ${table}.`);
}
if (!config.includes('sb_publishable_')) throw new Error('Configuração deve usar publishable key moderna.');
if (/sb_secret_|service_role/i.test(config + app + panels + html)) throw new Error('Segredo elevado encontrado no conteúdo público.');
...
console.log('Estrutura estática válida: autenticação, unidade, revisões, erros, desempenho e restrições do MVP conferidos.');
